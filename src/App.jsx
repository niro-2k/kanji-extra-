                  ...
                  <div className={`pl-2 border-l-2 ${darkMode ? 'border-rose-400/50' : 'border-rose-200'}`}>
                    {char.ondokuEx?.slice(0, 2).map((ex, i) => (
                      <div key={i} className="py-1">
                        <ruby className={`font-medium ${darkMode ? 'text-slate-100' : 'text-slate-800'}`} style={{ fontSize: '24px' }}>
                          {ex.word}
                          <rp>(</rp>
                          <rt className={`font-normal ${darkMode ? 'text-rose-400' : 'text-rose-500'}`} style={{ fontSize: '11px' }}>{ex.reading}</rt>
                          <rp>)</rp>
                        </ruby>
                        <span className={`text-sm ml-2 ${darkMode ? 'text-slate-400' : 'text-slate-600'}`}>{ex.meaning}</span>
                      </div>
                    ))}
                  </div>
                </>
                  ...
                  {char.kundokuEx?.slice(0, 2).map((ex, i) => (
                    <div key={i} className="py-1">
                      <ruby className={`font-medium ${darkMode ? 'text-slate-100' : 'text-slate-800'}`} style={{ fontSize: '24px' }}>
                        {ex.word}
                        <rp>(</rp>
                        <rt className={`font-normal ${darkMode ? 'text-sky-400' : 'text-sky-500'}`} style={{ fontSize: '11px' }}>{ex.reading}</rt>
                        <rp>)</rp>
                      </ruby>
                      <span className={`text-sm ml-2 ${darkMode ? 'text-slate-400' : 'text-slate-600'}`}>{ex.meaning}</span>
                    </div>
