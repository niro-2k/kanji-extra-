  ...
  return (
    ...
    >
      <div className="flex items-end gap-2">
        <div>
          <div className={`text-base transition-opacity leading-none ${revealed ? 'opacity-100' : 'opacity-0'} ${color}`}>
            {ex.reading}
          </div>
          <div className={`font-medium ${darkMode ? 'text-slate-100' : 'text-slate-800'}`} style={{ fontSize: '32px', lineHeight: 1.1 }}>
            {ex.word}
          </div>
        </div>
        <div className={`text-base transition-opacity pb-1 ${revealed ? 'opacity-100' : 'opacity-0'} ${darkMode ? 'text-slate-300' : 'text-slate-500'}`}>
          {ex.meaning}
        </div>
      </div>
    </div>
              ...
              <div className={`pl-3 border-l-2 ${darkMode ? 'border-rose-400/50' : 'border-rose-200'}`}>
                {hasOndokuEx ? char.ondokuEx.slice(0, 2).map((ex, i) => (
                  <ExampleItem
                    ...
                    darkMode={darkMode}
                    color={darkMode ? 'text-rose-200' : 'text-rose-500'}
                  />
                )) : (
              ...
              <div className={`pl-3 border-l-2 ${darkMode ? 'border-sky-400/50' : 'border-sky-200'}`}>
                {hasKundokuEx ? char.kundokuEx.slice(0, 2).map((ex, i) => (
                  <ExampleItem
                    ...
                    darkMode={darkMode}
                    color={darkMode ? 'text-sky-200' : 'text-sky-500'}
                  />
                )) : (
                  <div className="text-slate-600 text-sm">-</div>
                )}
