...
// 예: "打ち合わせ" → [{text:"打", isKanji:true}, {text:"ち", isKanji:false}, {text:"合", isKanji:true}, {text:"わせ", isKanji:false}]
...
// 한자 블록에 읽기를 매칭 (단어의 한자 부분만 루비 배치)
// word: "打ち合わせ", reading: "うちあわせ"
// → 한자"打"→"う", 히라가나"ち"→스킵, 한자"合"→"あ", 히라가나"わせ"→스킵
function matchReadings(word, reading) {
  const segments = splitWord(word);
  
  // 전부 한자인 경우 (打楽器 등) → 전체에 루비
  const allKanji = segments.every(s => s.isKanji);
  if (allKanji) {
    return [{ text: word, isKanji: true, ruby: reading }];
  }

  // 히라가나가 섞인 경우 → 뒤에서부터 히라가나 매칭
  const result = [];
  let readingLeft = reading;
      ...
      if (j === kanjiIndices.length - 1) {
        ...
        if (nextNonKanjiIdx >= 0) {
          ...
          if (splitPos >= 0) {
            ...
            rubyMap[idx] = remaining.slice(0, kanjiLen);
            remaining = remaining.slice(kanjiLen);
          }
        } else {
          rubyMap[idx] = remaining.slice(0, kanjiLen);
          remaining = remaining.slice(kanjiLen);
